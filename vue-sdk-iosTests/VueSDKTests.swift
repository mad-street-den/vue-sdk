...
final class eventTrackTests: XCTestCase {
    ...
    override func setUpWithError() throws {
        ...
        eventPresenter = EventPresenter(sdkservice: VueSDKService(apiClient: mockHTTPClient))
    }
...
final class searchRecommendationTests: XCTestCase{
    ...
    var mockService: MockVueSDKServiceable!
    
    override func setUpWithError() throws {
        mockService = MockVueSDKServiceable()
        recommendationPresenter = RecommendationPresenter(sdkservice: mockService)
    }
...
class VueSDKEndpointTests: XCTestCase {
    func testDiscoverEndpoint() {
        let endpoint = VueSDKEndpoint.discover
    ...
    func testTrackEndpoint() {
        ...
        let endpoint = VueSDKEndpoint.track(body: body, headers: headers)
    ...
    func testSearchEndpoint() {
        ...
        let endpoint = VueSDKEndpoint.search(body: body, headers: headers)
...
class MSDErrorTests: XCTestCase {
    func testMSDErrorInitialization() {
        ...
        let msdError = VueSDKError(requestId: "123", correlationId: "456", errors: apiError)
    ...
    func testMSDErrorErrorCode() {
        let errorMap = VueSDKError.errorCode(.ERR002)
